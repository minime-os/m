...
static int Zip_read_version(FILE* zip, char* out_rom_name, size_t max_len) {
	...
	while (1) {
		...
		char *name = base ? base + 1 : filename;
		if (strcmp(name, ".version") == 0) {
			...
			strcpy(tmp_template, "/tmp/minarch-version-XXXXXX");
			...
			if (extract(zip, dst, compressed_size) == 0) {
				fseek(dst, 0, SEEK_SET);
				if (fgets(out_rom_name, max_len, dst)) {
					char *end = out_rom_name + strlen(out_rom_name) - 1;
					while (end >= out_rom_name && (*end == '\r' || *end == '\n' || *end == ' ' || *end == '\t')) {
						*end = '\0';
						end--;
					}
					fclose(dst);
...
static void Game_open(char* path) {
	...
	if (suffixMatch(".zip", game.path)) {
		...
		int is_multiversion = 0;
		char version_rom_name[MAX_PATH] = {0};
		FILE *zip = fopen(game.path, "r");
		if (zip != NULL) {
			is_multiversion = Zip_read_version(zip, version_rom_name, sizeof(version_rom_name));
			fclose(zip);
		}
	
		// if the core doesn't support zip files natively, OR if it's a multi-version zip
		if (!supports_zip || is_multiversion) {
			FILE *zip = fopen(game.path, "r");
			if (zip==NULL) {
			...
			while (1) {
				...
				if (is_multiversion) {
					char *base = strrchr(filename, '/');
					char *name = base ? base + 1 : filename;
					if (strcmp(name, version_rom_name) == 0 || strcmp(filename, version_rom_name) == 0) {
						found = 1;
					}
				} else {
				...
				fclose(dst);
				strcpy((char*)game.name, basename(filename));
				break;
			}
	...
	.frontend = { // (OptionList)
		...
		.options = (Option[]){
			...
			[FE_OPT_REWIND_CAPTURE] = {
				...
				.name	= "Rewind Capture",
				.desc	= "Interval between rewind captures.\nLower values improve smoothness but cost more CPU.",
				.default_value = 0,
				.value = 0,
				.count = 9,
			...
			[FE_OPT_REWIND_KEYFRAME] = {
				...
				.desc	= "How often rewind stores a full keyframe.\nLower values improve seek cost but use more memory.",
				.default_value = 1,
				.value = 1,
				.count = 5,
			...
			[FE_OPT_BEZEL] = {
				.key	= "minarch_screen_bezel",
				.name	= "Screen Bezel",
				.desc	= "Show a decorative bezel overlay around the game frame.",
				.default_value = 0,
				.value = 0,
				.count = 2,
				.values = onoff_labels,
	...
	for (int i=0; i<SHORTCUT_COUNT; i++) {
		...
		if (!mapping->mod || PAD_isPressed(BTN_MENU)) {
			if (i==SHORTCUT_TOGGLE_FF) {
				...
				int hold_active = PAD_isPressed(btn);
				if (hold_active) {
					toggled_ff_on = 0;
					setFastForward(0);
				}
				setRewindPressed(hold_active);
				if (mapping->mod && hold_active) ignore_menu = 1;
				continue;
			}
...
static BezelPreset bezel_presets[] = {
	{"GB",  80, 24, 480, 432},
	{"GBC", 80, 24, 480, 432},
	{"GBA",  0, 40, 640, 400},
	{"FC",  80,  0, 480, 480},
...
static void Bezel_load(void) {
	...
	if (!bezel_enabled) return;
	
	char bezel_path[MAX_PATH];
	char tag[MAX_PATH];
	strcpy(tag, core.tag);
	...
	if (!exists(bezel_path)) {
		...
		for (int i = 0; lower_tag[i]; i++) {
			if (lower_tag[i] >= 'A' && lower_tag[i] <= 'Z') {
				lower_tag[i] += 32;
			}
		}
	...
	if (exists(bezel_path)) {
		LOG_info("Loading bezel: %s\n", bezel_path);
		SDL_Surface* loaded = IMG_Load(bezel_path);
		if (loaded) {
			bezel_surface = SDL_ConvertSurface(loaded, screen->format, 0);
			SDL_FreeSurface(loaded);
		} else {
			LOG_error("Error loading bezel image: %s\n", IMG_GetError());
		}
	} else {
		LOG_info("Bezel file not found for tag: %s\n", core.tag);
	}
}
...
static void selectScaler(int src_w, int src_h, int src_p) {
	...
	if (bezel_surface != NULL && bezel_enabled) {
		for (int idx = 0; bezel_presets[idx].tag != NULL; idx++) {
			if (strcmp(bezel_presets[idx].tag, core.tag) == 0) {
				renderer.dst_x = bezel_presets[idx].x;
				renderer.dst_y = bezel_presets[idx].y;
				renderer.dst_w = bezel_presets[idx].w;
				renderer.dst_h = bezel_presets[idx].h;
				renderer.scale = -1;
				renderer.aspect = core.aspect_ratio;
